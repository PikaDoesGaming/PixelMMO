...
public class Button {
	...
	public void tick() {
		...
		if(MouseHandler.hasClicked() && containsMouse()) {
			pressed = true;
		}
		if(!MouseHandler.isLeftButton()) {
	...
	private boolean containsMouse() {
		if(MouseHandler.getX() >= x && MouseHandler.getX() <= x + width
				&& MouseHandler.getY() >= y && MouseHandler.getY() <= y + height) {
			return true;
		}
		return false;
	}
